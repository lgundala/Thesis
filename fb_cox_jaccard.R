...
for(i in 0:9){
  for(j in 0:4){
    ...
    coxmodel <- cph(Surv(time,status)~jaccard,data=df_train,surv=TRUE)
    ttt <- c(1,2,3,4,5)
    pf<-predictSurvProb(coxmodel,newdata=df_test,times=ttt)
    k<- paste("fb",toString(i),"survProbj",toString(j),".csv",sep="")
    write.table(pf,file=k,sep='\t',na="NA",dec=".",row.names=FALSE,col.names=TRUE)
    ApparrentCindex <- pec::cindex(pf,formula=Surv(time,status)~jaccard,data=df_test,eval.times=ttt)
    k<- paste("fb",toString(i),"cindexj",toString(j),".csv",sep="")
